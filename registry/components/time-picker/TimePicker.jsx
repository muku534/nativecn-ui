...
function ClockIcon({ color, size = 18 }) {
  return (
    <View
      style={{
        width: size,
...
const TimeItem = ({ item, index, activeIndexContinuous, textPrimary }) => {
  ...
  return (
    <Animated.View style={[styles.timeItem, itemStyle]}>
      <Text numberOfLines={1} style={[styles.timeText, { color: textPrimary }]}>{item.label}</Text>
    </Animated.View>
  );
};
...
export default function TimePicker({
  ...
  const scrollViewStartRef = useAnimatedRef();
  const scrollViewEndRef = useAnimatedRef();
  ...
  const getHandRotation = (continuousIndex, isHour) => {
    'worklet';
  ...
  const listContainerStyle = useAnimatedStyle(() => {
    return {
      ...
      transform: [
        { translateX: interpolate(rangeAnim.value, [0, 1], [0, 10]) }
      ],
    };
  });
  ...
  const endListStyle = useAnimatedStyle(() => {
    return {
      ...
      right: 0,
      opacity: rangeAnim.value,
      width: interpolate(rangeAnim.value, [0, 1], [0, 100]),
      height: '100%',
      overflow: 'hidden',
      alignItems: 'center',
    };
  ...
  const startListStyle = useAnimatedStyle(() => {
    return {
      position: 'absolute',
      left: 0,
      width: interpolate(rangeAnim.value, [0, 1], [130, 100]),
      height: '100%',
      overflow: 'hidden',
      alignItems: 'center',
    };
      ...
      <Animated.View style={[styles.expandedContent, expandedContentStyle]}>
        ...
        <View style={styles.mainRow}>
          ...
          <Animated.View style={[styles.listContainer, listContainerStyle]}>
            ...
            {/* Range Dash Indicator */}
            <Animated.View style={dashStyle} pointerEvents="none">
              <Text style={[styles.timeText, { color: textSecondary }]}>-</Text>
            </Animated.View>
